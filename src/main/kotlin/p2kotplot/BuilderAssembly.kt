...
class BuilderAssembly(val builder: PublicFlatBuilderRepresentation) {
    ...
    fun assembleAll(): KotlinApi {
        val initialTopLevelFunctions = builder.builderFunctions.filter { it.inClass == null }
...
//    fun assemble(enum : Enum) =
    ...
    private fun BuilderFunction.getBody(parametersOfOverload: List<ParameterComponents>): String {
        ...
        return when {
            ...
            // Data class
            this.inClass != null -> dataClassFunctionBody(objectConstruction, this.name)
            // Top level
            else -> topLevelFunctionBody(objectConstruction)
        }
    }
}
